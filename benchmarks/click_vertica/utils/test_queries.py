...
QUERIES = {
    'average_movies_per_user': ("""select avg(movies_watched) 
    from ( 
    ...
    'top_20_users_by_total_view_time': ("""select user_id, sum(viewed_frame) as view_time 
    from views 
    group by user_id 
    order by view_time desc 
    limit 20;"""),
    'top_20_movies_by_view_time': ("""select movie_id, max(viewed_frame) as view_time
    from views
    group by movie_id
    order by view_time desc
    limit 20;"""),
